...
function drawByPicType(_context,_canvasPic){
	...
	if(currPicType==1){
		//heart
		drawHeart(_context,_canvasPic,0.4,"pink");
		...
		//Fan风扇
		drawFan(_context,_canvasPic,"red",3);
		...
		//径向渐变圆
		drawRadialCircle(_context,_canvasPic,2*Math.PI/180);
		...
		//返回图标
		drawBackward(_context,_canvasPic,2*Math.PI/180);
	}
}//end function
...
function drawLineClock(_context,_canvasPic,_fillColor){
	...
	_context.fillRect(fPointArray[0],fPointArray[1], this.width, this.width);
}
